...
import { Plus, Minus, History, Clock, User, X, MessageSquare, UtensilsCrossed, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function OnlineOrder() {
  ...
  const startPayment = async () => {
    ...
    try {
      ...
      showNotif("Terjadi kesalahan sistem. Silakan coba beberapa saat lagi."); 
      setLoading(false); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-black italic uppercase p-4 pb-44 text-slate-800 tracking-tighter">
      ...
      <div className="bg-indigo-600 p-6 rounded-[2.5rem] text-white mb-6 shadow-xl">
        <label className="text-[9px] mb-2 block opacity-80 uppercase font-black">Nama Penerima Pesanan:</label>
        <div className="relative">
          <User className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-300" size={18} />
          <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Masukkan Nama Anda" className="w-full bg-white/10 border-2 border-white/20 rounded-2xl py-4 pl-12 pr-4 outline-none font-black italic uppercase placeholder:text-indigo-300 shadow-inner"/>
        </div>
      </div>
      ...
      {cart.length > 0 && !isCartOpen && (
        <button onClick={() => setIsCartOpen(true)} className="fixed bottom-6 left-4 right-4 bg-indigo-600 text-white p-5 rounded-[2.5rem] shadow-2xl flex justify-between items-center z-[100] border-b-4 border-indigo-800 active:scale-95 transition-all">
          <div className="flex flex-col items-start ml-4 text-left font-black italic">
            <span className="text-[8px] opacity-80 uppercase tracking-widest">Total Tagihan</span>
            <span className="text-lg tracking-tighter leading-none">Rp {cart.reduce((a, b) => a + b.price * b.qty, 0).toLocaleString()}</span>
          </div>
          <div className="bg-white/20 px-5 py-3 rounded-2xl text-[9px] border border-white/10 uppercase italic font-black">Keranjang</div>
        </button>
      )}
              ...
              {cart.map(item => (
                <div key={item.id} className="border-b border-slate-50 pb-4 italic uppercase font-black">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Tambahkan Catatan Pesanan" value={item.note || ''} onChange={(e) => setCart(cart.map(c => c.id === item.id ? {...c, note: e.target.value} : c))} className="w-full bg-slate-50 rounded-2xl py-3 px-10 text-[8px] outline-none border-2 border-transparent focus:border-indigo-100 font-black italic uppercase placeholder:text-slate-300"/>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showStatusModal && activeOrder && (
        <div className="fixed inset-0 bg-black/95 z-[300] flex items-center justify-center p-6 italic uppercase font-black text-center transition-all">
          <div className="bg-white rounded-[3.5rem] p-10 w-full max-w-sm border-[12px] border-slate-50 shadow-2xl animate-in zoom-in">
            ...
            <h2 className="text-xl mb-1 italic tracking-tighter leading-none uppercase">Order ID: {activeOrder.id}</h2>
