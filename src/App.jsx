...
export default function OnlineOrder() {
  ...
  const subscribeOrder = (id) => {
    supabase.channel(`order-${id}`).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `id=eq.${id}` }, (p) => {
      ...
      // Jika pesanan diselesaikan (completed) oleh admin, hapus dari local storage
  ...
  const startPayment = async () => {
    if (!customerName.trim()) return setNotification({ show: true, msg: 'Mohon masukkan nama Anda.', type: 'error' });
    setLoading(true);
    const orderId = `ACUN-${Date.now().toString().slice(-4)}`;
    const totalAmount = cart.reduce((a, b) => a + b.price * b.qty, 0);

    try {
      // 1. Minta Token Midtrans (Data pesanan belum masuk ke DB)
      ...
      if (!data.token) throw new Error("Sistem pembayaran sedang sibuk.");

      // 2. Buka Midtrans Snap
      window.snap.pay(data.token, {
        onSuccess: async () => {
          // 3. LOCK: Hanya simpan ke DB kalau pembayaran BERHASIL
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-black italic uppercase p-4 pb-48 text-slate-800 tracking-tighter">
      ...
      <div className="bg-indigo-600 p-6 rounded-[2.5rem] text-white mb-6 shadow-xl">
        <label className="text-[9px] mb-2 block opacity-80">Nama Anda:</label>
        <div className="relative">
          <User className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-300" size={18} />
          <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Masukkan Nama" className="w-full bg-white/10 border-2 border-white/20 rounded-2xl py-4 pl-12 pr-4 outline-none font-black italic uppercase placeholder:text-indigo-300"/>
        </div>
      </div>
      ...
      {/* ✅ TASKBAR BAWAH (GANTI FLOATING BANNER) */}
      <div className="fixed bottom-0 left-0 right-0 z-[100] flex flex-col pointer-events-none">
        {/* Status Slim Bar */}
        {activeOrder && (
            <button 
                onClick={() => setShowStatusModal(true)} 
                className="pointer-events-auto mx-4 mb-2 bg-white border-2 border-indigo-600 p-3 rounded-2xl shadow-2xl flex items-center justify-between animate-in slide-in-from-bottom duration-500"
            >
                <div className="flex items-center gap-3">
                    <div className="bg-indigo-100 p-2 rounded-lg text-indigo-600">
                        <Clock size={16} className={activeOrder.status === 'on_process' ? 'animate-spin' : ''} />
                    </div>
        ...
        {cart.length > 0 && !isCartOpen && (
            <button 
                ...
                className="pointer-events-auto bg-slate-900 text-white p-6 rounded-t-[3rem] shadow-2xl flex justify-between items-center border-t border-slate-800 active:scale-95 transition-all"
            >
                <div className="flex items-center gap-4 ml-2">
                    <ShoppingBag size={24} className="text-indigo-400" />
                    <div className="text-left leading-none font-black italic">
                        <span className="text-[8px] opacity-60 block mb-1">TOTAL TAGIHAN</span>
                        <span className="text-lg">Rp {cart.reduce((a,b)=>a+b.price*b.qty,0).toLocaleString()}</span>
                    </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-[250] flex items-end">
          <div className="bg-white w-full rounded-t-[3.5rem] flex flex-col max-h-[90vh] animate-in slide-in-from-bottom duration-300 shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto px-8 py-4 space-y-6 font-black italic">
              {cart.map(item => (
                <div key={item.id} className="border-b border-slate-50 pb-4 uppercase">
                  ...
                  <input type="text" placeholder="TAMBAH CATATAN" value={item.note || ''} onChange={(e) => setCart(cart.map(c => c.id === item.id ? {...c, note: e.target.value} : c))} className="w-full bg-slate-50 rounded-2xl py-3 px-5 text-[8px] outline-none border-2 border-transparent focus:border-indigo-100 font-black italic uppercase"/>
                </div>
              ))}
            </div>
            <div className="bg-slate-50 p-10 rounded-t-[3.5rem] border-t border-slate-100 font-black italic">
              ...
              <button onClick={startPayment} disabled={loading} className="w-full py-6 bg-indigo-600 text-white rounded-[2rem] shadow-xl font-black italic tracking-widest active:scale-95 disabled:opacity-50 uppercase">{loading ? "SEDANG PROSES..." : "KONFIRMASI & BAYAR"}</button>
            </div>
          </div>
        </div>
      )}

      {/* MODAL STATUS PESANAN (DIGITAL RECEIPT) */}
      {showStatusModal && activeOrder && (
        <div className="fixed inset-0 bg-black/95 z-[300] flex items-center justify-center p-6 text-center">
          <div className="bg-white rounded-[3.5rem] p-10 w-full max-w-sm border-[12px] border-slate-50 shadow-2xl animate-in zoom-in">
            ...
            <h2 className="text-xl mb-1 italic font-black uppercase leading-tight tracking-tighter">ORDER ID: {activeOrder.id}</h2>
            <p className="text-[8px] text-slate-400 mb-6 font-black uppercase">Tunjukkan kode ini saat pengambilan</p>
